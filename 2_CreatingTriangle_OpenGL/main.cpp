...
Description:
This program demonstrates the fundamentals of the modern OpenGL rendering
pipeline by drawing a single triangle on the screen.

The application initializes an OpenGL 3.3 Core Profile context using GLFW,
loads OpenGL functions with GLAD, compiles and links a vertex shader and a
fragment shader into a shader program, uploads vertex data to the GPU through
a Vertex Buffer Object (VBO), configures a Vertex Array Object (VAO), and
renders the geometry using glDrawArrays() with the GL_TRIANGLES primitive.

This example introduces the essential components required for modern GPU
rendering and serves as a foundation for more advanced topics such as
transformations, textures, lighting, camera systems, and 3D graphics.
*/
...
int main() {
	...
	*/
	...
	GLfloat vertices[] =
	{
		-0.4f, -0.6f * float(sqrt(3)) / 3, 0.0f,
		0.6f, -0.4f * float(sqrt(3)) / 3, 0.0f,
		0.0f, 0.3f * float(sqrt(3)) * 2 / 3, 0.0f,
	};
