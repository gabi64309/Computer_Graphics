...
Description:
This program initializes an OpenGL 3.3 Core Profile context using GLFW
and GLAD. It demonstrates the modern OpenGL rendering pipeline by creating
a Vertex Buffer Object (VBO), a Vertex Array Object (VAO), compiling and
linking vertex and fragment shaders, and rendering three vertices as points.

Each point is rendered with a fixed size using the built-in variable
gl_PointSize in the vertex shader, while the fragment shader assigns a
constant orange color to every fragment. The application also includes
basic fail-fast error handling during GLFW and GLAD initialization.
